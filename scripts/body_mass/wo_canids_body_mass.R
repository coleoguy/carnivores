...
for(i in 1:length(drop.species)){
  store.drop[i] <- which(bm$species == drop.species[i])
}

bm_new <- bm[-c(store.drop),]
...
for(i in 1:nrow(bm_new)){
  hit <- which(chroms$species == bm_new$species[i])
  if(length(hit) > 1){
    hit <- sample(hit, 1)
  }
...
rm(chroms, cur.tree, bm, bm_new, tree, trees, drop.species, i, missing, 
   store.drop, hit)

###DISCRETIZE RANGE SIZES###----------------------------------------------------
#discretize range size based on the median
x <- median(dat.pruned$body.mass)
dat.pruned$body.mass <- as.numeric(dat.pruned$body.mass >= x)
...
write.csv(temp, "../../results/body_mass/bm_wocanid.csv")
