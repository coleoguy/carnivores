...
chromplus <- read.csv("/Users/michelle/Desktop/Carninvore/results_25small75large/carn_data_all.csv")
...
states <- 1:ncol(p.mat)
names(states) <- colnames(p.mat)
colnames(parMat) <- rownames(parMat) <- colnames(p.mat) <- states
...
p.mat <- p.mat[tree$tip.label,]
#sort MCMC data to match with the tree tip order
rownames(dat.pruned) <- dat.pruned$species
dat.pruned <- dat.pruned[tree$tip.label, ]
...
# make states
rng <- range(dat.pruned$hap.chrom) + c(-2,2)
...
for(i in 1:Ntip(tree)){
  dat.tips$tip[i] <- i
  dat.tips$chrom[i] <- dat.pruned$hap.chrom[dat.pruned$species == tree$tip.label[i]]
}
...
dat.pruned$tipRates <- tip.branches[,3]
